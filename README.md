...
- The list detail page loads its initial data once per visit; loading or switching translations does not repeat the requests. Entries appear when the list, entries and history responses are ready, without waiting for owner-visible member data. Members have a separate loading indicator in the sharing sheet; a failed member request shows an error while keeping the list and entries visible. Access-denied messages follow the current language without refetching the list.
