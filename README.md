...
- The list detail view uses a sticky top bar, filters open items by name or optional details through an always-visible search field, provides a more-options flyout for rename and sharing, and includes a bottom-sheet add-item flow with an overlaid autocomplete suggestion dropdown that anchors to the input, an inline icon preview to the right of the field, a smoothly sliding icon browser, outside-tap dismissal, swipe-to-delete entry rows with optional detail text, and a recently used panel that updates immediately when items are completed, preserves optional details when re-adding history items, and lets users dismiss history chips by deleting matching completed entries for every connected list member.
