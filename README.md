...
- The overview refetches lists when list rename/delete SSE events arrive, and the list detail page refetches the active list's entries, members, or recently used history when matching entry/member/history SSE events arrive over `GET /api/events?token=<jwt>`.
