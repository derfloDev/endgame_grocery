...
| T-003 | Reachability-based connectivity: `/api/health` probe with timeout, rate limiting and in-flight de-duplication as the source of truth, `navigator.onLine` demoted to a hint, offline queue triggered additionally on `pageshow` and `focus`. See `.ai/PLAN.md` Task T-003. | done | `navigator.onLine === true` with a failing probe reports offline and keeps queueing; `navigator.onLine === false` with a succeeding probe reports online and drains; repeated wake-ups issue at most one probe per 5 s window; an open SSE stream reports online without a probe. | Review round 1 `PASS_WITH_NOTES`, no required fixes. Lint, build and full tests pass twice on an idle machine (558 frontend / 174 backend); no new type errors; T-002 resync e2e still green. Live Chromium checks: burst of 5 wake-ups issued 0 extra probes inside the window and exactly 1 after expiry, and an unreachable server with `navigator.onLine === true` raised the offline banner, then cleared it on recovery without a reload. See `.ai/REVIEW.md` T-003. | none |
| T-004 | Request timeouts and queue retry: 10 s timeout on every request including the drain `fetch`, aborts classified as network errors, transient (network/timeout/5xx) failures retried with capped backoff, 4xx kept permanent with the discard action, stalled-drain guard release, order and idempotency preserved across retries. See `.ai/PLAN.md` Task T-004. | done | A drain failing with a network error or 5xx retries automatically with increasing delay and no user interaction; a 4xx does not retry and still offers discard; a request past the timeout is treated as offline so reads fall back to cache and writes are queued; a drain interrupted mid-queue resumes in original order without resending synced mutations; a stalled drain cannot block the queue past 30 s. | Review round 1 `PASS_WITH_NOTES`, no required fixes. Lint, build and full tests pass twice on an idle machine (579 frontend / 174 backend); no new type errors; 4/4 browser specs pass including the reload test proving an accepted parent is not resent. `completeOfflineMutation` additionally verified against real IndexedDB: remaining mutations survive with correctly merged temporary-ID mappings. See `.ai/REVIEW.md` T-004. | none |
