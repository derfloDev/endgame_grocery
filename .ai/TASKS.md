...
| T-002 | Frontend: remove deleteFromHistory, remove dismiss button from RecentlyUsedSection, remove dismissRecentlyUsedEntry from useListDetailData, update frontend tests | done | deleteFromHistory export removed; dismiss button absent from RecentlyUsedSection UI; dismissRecentlyUsedEntry removed from useListDetailData return value; ListDetailPage no longer passes onDismiss; all frontend tests pass | `npm run test --workspace frontend -- RecentlyUsedSection ListDetailPage app.test feature-components` pass; `npm run lint` pass with existing frontend fast-refresh warning; `npm run build` pass with existing Vite chunk-size warning; `npm test` pass | none |
