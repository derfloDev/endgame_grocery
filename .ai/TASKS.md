...
| T-009 | Spacing-scale tokens & consistency fixes | ready_for_implement | `--space-1` through `--space-12` defined in `tokens.css`; 11 spacing properties in `index.css` updated to use tokens or correct values; `npm run lint` passes; `npm run build` passes; `npm test` passes | n/a | implement |
