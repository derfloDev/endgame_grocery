...
| T-007 | PWA & Offline Support | done | Lighthouse PWA audit passes (installable, SW registered, manifest present); app reads data offline from IndexedDB; queued writes sync on reconnect; offline banner shown | `npm run lint` (passes with one non-blocking React fast-refresh warning); `npm run build` (generates `manifest.webmanifest` and `sw.js`); `npm test` | none |
