...
| T-007 | Frontend: place item-icon preview to the right of the input field | done | Icon/spinner appears inline to the right of the input, not below it; dropdown still spans input width; tests pass | `npm run test --workspace frontend -- src/components/AddItemSheet.test.jsx src/components/AutocompleteSuggestions.test.jsx` pass; `npm run lint` pass (1 existing warning); `npm run build` pass; `npm test` pass | none |
| T-008 | Frontend: smooth slide animation for icon browser open/close | done | Icon browser expands with smooth slide-down; collapses with slide-up; no abrupt jump; tests pass | `npm run test --workspace frontend -- src/components/AddItemSheet.test.jsx src/app.test.jsx` pass; `npm run lint` pass (1 existing warning); `npm run build` pass; `npm test` pass | none |
