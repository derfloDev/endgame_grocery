...
| T-007 | Hotfix migration: fix `pending_push_jobs.items` default — change `DEFAULT ARRAY[]` to `DEFAULT '[]'::jsonb` in `1713920400000_add_push_tables.cjs` | done | `npm run migrate` completes without error; `items` column default is valid JSONB | `npm run test --workspace backend -- src/db/migrations.test.js`, `npm run migrate`, `npm run lint`, `npm run build`, `npm test` | none |
