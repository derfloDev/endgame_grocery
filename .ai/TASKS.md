...
| T-008 | Backend: v1 API – HA-Statusmapping entfernen, rohe DB-Werte zurückgeben | ready_for_implement | Item-Status in allen v1-Responses ist `open` oder `done` (keine Umwandlung in `needs_action`/`completed`); OpenAPI-Spec zeigt Enum `open \| done`; Tests grün | n/a | implement |
