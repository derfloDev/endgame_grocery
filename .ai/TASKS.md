...
| T-004 | Move pages to sub-folders + CSS Modules: LoginPage, RegisterPage, ForgotPasswordPage, ResetPasswordPage, VerifyEmailPage, InviteAcceptPage (all import `styles/auth.module.css`), OverviewPage, ListDetailPage, SearchPage; update App.tsx with `app-shell` module | done | Each page has `PageName/PageName.tsx` + module (where applicable); auth pages import shared `auth.module.css`; `detail-page` / `overview-page` wrapper classNames cleaned up or replaced with module locals; all import paths in `App.tsx` and router updated; `npm run build` + `npm test` pass | `npm run lint`; `npm run test --workspace frontend -- pages/page-components.test.ts pages/ListDetailPage.test.tsx styles/shared.test.ts app.test.tsx`; `npm run build`; `npm test` | none |
| T-005 | Remove all extracted component-specific styles from `index.css`; retain only resets, base typography, `#root`, and `@keyframes` | done | `index.css` contains ≤ 40 lines (only resets + animations); `styles/shared.css` is the new home for design-system classes; no visual regressions; `npm run lint` + `npm run build` + `npm test` all pass | `npm run lint`; `npm run test --workspace frontend -- styles/index-cleanup.test.ts styles/shared.test.ts components/AddItemSheet/AddItemSheet.test.tsx pages/ListDetailPage.test.tsx pages/page-components.test.ts app.test.tsx`; `npm run build`; `npm test` | none |
