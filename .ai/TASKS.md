...
| T-008 | Frontend: update worker + hook to return icon name and top matches | ready_for_implement | Hook returns `{ iconName, topMatches, loading }`; exact match → `topMatches: []`; async path returns ≤5 topMatches strings; unit tests pass | n/a | implement |
| T-011 | Frontend: `IconPickerSheet` shared full-catalogue picker component | ready_for_implement | All registry icons render in grid; search filters list; tapping fires `onSelect`; selected icon highlighted; tests pass | n/a | implement |
| T-009 | Frontend UI: `AddItemSheet` inline picker + "Mehr anzeigen" → `IconPickerSheet` | ready_for_implement | SVG preview renders; alternatives row visible for semantic matches; "Mehr anzeigen" opens `IconPickerSheet`; full picker selection updates preview; `onAdd` passes selected icon name; tests pass | n/a | implement |
