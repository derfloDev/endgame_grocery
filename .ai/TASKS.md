...
| T-003 | Update deprecated GitHub Actions to Node.js 24-compatible versions in `ci.yml` and `release-please.yml` | done | No Node.js 20 deprecation warnings; no `google-github-actions/release-please-action` deprecation warning; all CI jobs pass; all action versions updated per plan table | Updated `ci.yml` and `release-please.yml` to current maintained majors; replaced deprecated `google-github-actions/release-please-action` with `googleapis/release-please-action@v5`; documented the maintained-action policy in `README.md`; `npm run lint`; `npm run build`; `npm test`; approved for commit in review | none |
