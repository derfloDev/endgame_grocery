...
| T-006 | Config + docs: add `useCredentials: true` to VitePWA so manifest fetch sends CF_Authorization cookie; document CF Access bypass requirement for SW scripts in README | ready_for_implement | Built `index.html` manifest link contains `crossorigin="use-credentials"`; README has Cloudflare Access subsection with bypass table; `npm run build` passes | n/a | implement |
