...
| T-007 | Add explicit branch filters to CI workflow so `main` builds and release-please trigger reliably after PR merge | done | (1) `ci.yml` has `branches: [main]` on `push` and `pull_request` triggers. (2) No other changes. (3) `npm run lint` passes. | `npm run lint` (pass; existing AuthContext Fast Refresh warning); visual diff check confirms only `branches: [main]` was added to `push` and `pull_request` in `.github/workflows/ci.yml`. | none |
| T-006 | Restore missing bottom spacing below API-key section ("Neu generieren" button) | done | (1) ≈ 16 px gap visible between "Neu generieren" and the Logout divider line. (2) No other spacing changes. (3) lint + build + test green. | `npm run lint` (pass; existing AuthContext Fast Refresh warning), `npm run test --workspace frontend -- InfoSheet` (pass: 14 tests), `npm run build` (pass; existing Vite chunk-size warning), targeted retries for earlier full-suite timeouts passed (`app.test.tsx` detail-sheet test and `AddItemSheet.test.tsx` icon-browser test), `npm test` (pass: frontend 423, backend 164) | none |
