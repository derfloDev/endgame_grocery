...
| T-002 | Gate release on CI success: change `release-please.yml` trigger from `push` to `workflow_run` with CI success condition | ready_for_implement | `release-please.yml` uses `workflow_run` trigger; release-please job has `if: github.event.workflow_run.conclusion == 'success'`; a failing CI run on `main` does not produce a new release or Docker image | n/a | implement |
