...
| T-003 | Show logged-in user at top of Info & Settings sheet | ready_for_implement | `display_name` and `email` visible above the logout button immediately after login AND after page reload with a pre-existing JWT session that has no stored user data | n/a | implement |
