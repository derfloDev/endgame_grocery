...
| T-005 | Frontend: export `resolveIconName` + `ICON_ALIASES` + `formatIconName` from iconRegistry; refactor EntryRow, RecentlyUsedSection, AutocompleteSuggestions, AddItemSheet to use shared resolution; icon picker shows human-readable labels without "Icon" prefix; add iconRegistry unit tests | done | `resolveIconName` unit-tested (null→null, known→same, unknown→null); `formatIconName` unit-tested (prefix stripped, PascalCase split, digit separated); icon browser and suggested-icon picker show formatted labels; AddItemSheet tests updated for new aria-labels; all tests pass | `npm run test --workspace frontend -- src/data/iconRegistry.test.js src/components/AddItemSheet.test.jsx src/components/entry-row.test.jsx src/components/RecentlyUsedSection.test.jsx src/components/AutocompleteSuggestions.test.jsx`, `npm run lint` (pass with existing frontend warning), `npm run build`, `npm test` | none |
| T-006 | Config + docs: add `useCredentials: true` to VitePWA so manifest fetch sends CF_Authorization cookie; document CF Access bypass requirement for SW scripts in README | done | Built `index.html` manifest link contains `crossorigin="use-credentials"`; README has Cloudflare Access subsection with bypass table; `npm run build` passes | `npm run test --workspace frontend -- src/vite-config.test.js` (pass, required escalation because sandbox blocked `esbuild` spawn), `npm run lint` (pass with existing frontend warning), `npm run build`, `npm test`, `frontend/dist/index.html` contains `crossorigin="use-credentials"` on the manifest link | none |
