...
| T-008 | Feature components: migrate 13 components in `src/components/` to `.tsx`; all props typed, domain types from `src/types.ts` | done | Old `.jsx` files deleted; every component has props interface; build + lint + tests pass | `npm run lint`, `npx tsc -p frontend/tsconfig.json --noEmit`, `npm run build`, focused component Vitest command, `npm test` pass | none |
| T-009 | Pages and entry points: migrate 9 pages + `recentlyUsedState` to `.tsx`/`.ts`; migrate `App.tsx`, `main.tsx` | done | Old `.jsx`/`.js` files deleted; all `useState` calls explicitly typed; `document.getElementById` non-null asserted; build + lint + tests pass | `npm run lint`, `npx tsc -p frontend/tsconfig.json --noEmit`, `npm run build`, `npm run test --workspace frontend -- src/app.test.jsx src/pages/ListDetailPage.test.jsx src/pages/recentlyUsedState.test.js`, `npm test` pass; source scan confirms no old page/App `.jsx`/`.js` files remain | none |
| T-010 | Test files: rename 24 test files from `.jsx`/`.js` to `.tsx`/`.ts`; fix TS errors in mocks and helpers | done | Old test files deleted; `npm test` green with zero TS errors | `npx tsc -p frontend/tsconfig.json --noEmit`, `npm run lint`, `npm run build`, `npm test` pass; source scans confirm no old `.test.jsx`/`.test.js` files and no test `any` or TS suppressions remain | none |
