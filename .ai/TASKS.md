...
| T-006 | AddItemSheet layout fixes — rework 2: add min-height: 0 to .add-item-icon-browser-inner (base + browser-open) | done | Before "Mehr anzeigen": no empty gap, sheet not scrollable, gap below buttons ≤ 20 px; after "Mehr anzeigen": icon grid scrollable with visible scrollbar, Cancel/Add Item buttons always visible; closing browser returns to compact state | `npm run lint`; `npm run build`; `npm test` | none |
