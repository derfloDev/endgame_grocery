...
| T-010 | Fix badge overflow and done-item section — corner-flush badge (overflow:hidden), done+changed entries in recently-used section | ready_to_commit | Badge inside tile corner (no overflow); done+changed entries not in open section; done badge in recently-used; tests pass | `npm run test --workspace frontend -- EntryTile RecentlyUsedSection ListDetailPage recentlyUsedState` passed; `npm run lint` pass; `npm run build` pass; `npm test` pass — commit absorbs T-009 changes (same files, combined review PASS) | implement |
| T-011 | Fix badge corner radius (calc(--radius-md - 1px)), sharp bottom-left (0), self-done badge on toggle | ready_for_implement | Badge top-right seamlessly flush (no clip artefact); bottom-left radius 0; Done badge appears immediately when user themselves completes an item; tests pass | n/a | implement |
