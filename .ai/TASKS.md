...
| T-003 | Frontend: EventSourceContext + useListEvents-Hook | done | EventSource wird bei Token geöffnet und bei Logout geschlossen; Handler werden für eingehende Events aufgerufen; listId-Filter in useListEvents funktioniert; EventSourceProvider in main.jsx eingebunden | `npm run lint`; `npm run build`; `npm test`; `npm run test --workspace frontend -- src/components/AddItemSheet.test.jsx src/app.test.jsx src/context/EventSourceContext.test.jsx src/hooks/useListEvents.test.js` | none |
| T-004 | Frontend: OverviewPage + ListDetailPage reagieren auf SSE-Events | done | list:updated/list:deleted → loadLists() in OverviewPage; entry:*/member:* → gezielter Refetch in ListDetailPage; Handler-Referenzen sind stabil (useCallback) | `npm run lint`; `npm run build`; `npm test`; `npm run test --workspace frontend -- src/app.test.jsx` | none |
