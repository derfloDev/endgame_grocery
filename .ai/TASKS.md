...
| T-003 | Fix real-time sync: await history upsert before SSE broadcast in v1.js; re-fetch history on `entry:updated` SSE in frontend | done | (1) External done-toggle → item appears in "Zuletzt Verwendet" without reload; (2) External open-toggle → item disappears without reload; (3) Backend broadcasts only after history commit; (4) Frontend `entry:updated` triggers history re-fetch; (5) lint, build, test green | `node --test src/v1.test.js`; `npm run test --workspace frontend -- ListDetailPage`; `npm run lint`; `npm run build`; `npm test` | none |
