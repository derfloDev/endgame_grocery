...
| T-007 | Hotfix migration: fix `pending_push_jobs.items` default — change `DEFAULT ARRAY[]` to `DEFAULT '[]'::jsonb` in `1713920400000_add_push_tables.cjs` | ready_for_implement | `npm run migrate` completes without error; `items` column default is valid JSONB | n/a | implement |
| T-006 | Fix Docker publish pipeline: split docker-publish out of release-please.yml into a dedicated docker-publish.yml triggered by `on: release: types: [published]`; remove docker-publish job from release-please.yml | done | Merging a future release PR causes a Docker image to be built and pushed to GHCR without manual intervention; release-please.yml no longer contains docker-publish job; docker-publish.yml exists and triggers on release published event | `npm run test --workspace backend -- src/releaseWorkflow.test.js`, `npm run lint`, `npm run build`, `npm test` | none |
