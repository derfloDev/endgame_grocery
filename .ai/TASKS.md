...
| T-003 | Fix SW activation failure and redundant serviceWorker.ready wait in dev mode | done | SW activates in dev (DevTools shows "activated and running"); subscribe/unsubscribe cycle completes on localhost; subscribe() rejects with clear error within 8 s when SW unavailable; lint + build + tests pass | `npm run lint`; `npm run build`; `npm test` | none |
