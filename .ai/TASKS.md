...
| T-002 | Backend: API-Key-Management-Endpoints + `createRequireApiKey`-Middleware | ready_for_implement | `GET /api/auth/api-key` liefert aktuellen Key (null wenn keiner); `POST /api/auth/api-key` erzeugt/erneuert Key; Middleware setzt `req.user.sub` bei gültigem Header, 401 sonst; alle Tests grün | n/a | implement |
| T-003 | Backend: Externe REST-API v1 (5 Endpunkte) | ready_for_implement | Alle 5 Endpunkte unter `/api/v1/` funktionieren mit gültigem `X-Api-Key`; Status-Mapping `open`↔`needs_action`, `done`↔`completed` korrekt; 401 bei fehlendem/ungültigem Key; 403 bei fremder Liste; 404 bei unbekanntem Item; alle Tests grün | n/a | implement |
