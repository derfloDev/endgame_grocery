...
| T-008 | Remove obsolete swipe-to-delete E2E test | done | `swipeEntryLeft` helper and the `deletes an item from a shopping list via swipe` test block removed from `e2e/lists.spec.js`; remaining tests unaffected; CI E2E suite passes | `npm run lint`; `npm run build`; `npm test` | none |
